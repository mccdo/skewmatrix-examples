...
vec3 MapToNormal(sampler2D Map, vec2 MapCoord, float scale)
{
...
vec3 fBMToNormal(vec2 MapCoord, float octaves, float scale)
{
...
void main( void )
{
    ...
    // Show xy part of normal as red and green.
    //gl_FragData[0]=vec4( fvNormal.xy * 3., 0., 1.);
    //return;

    float fNDotL           = max( dot( fvNormal, fvLightDirection ), 0. );
